...
%     size(images)  28          28           1       60000
%     size(testimages)  28          28           1       10000
    it = 0;
    C = [];
    ...
    m = length(labels);
    numLayers = numel(cnn.layers);%返回数组A中元素个数。若是一幅图像，则numel(A)将给出它的像素数
    for e = 1:epochs
        % randomly permute indices of data for quick minibatch sampling
        rp = randperm(m);%生成1-60000整数的随机顺序
        for s=1:minibatch:(m-minibatch+1)%initval:step:endval
            it = it + 1;
            % momentum enable
            if it == momIncrease
                mom = momentum;
